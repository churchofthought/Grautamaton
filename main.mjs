...
window.onload = async () => {
	...
	const bindUniverse = idx => {
		...
		for (var x = 0; x < constants.CANVAS_WIDTH; x+= 5)
			for (var y = 0; y < constants.CANVAS_HEIGHT; y+= 5) {
				arr[4 * mousePosToArr(x, y)] = Math.floor((Math.random() > 0.5 ? 1 : -1) * Math.round(Math.random() * 25))
				arr[4 * mousePosToArr(x, y) + 1] = Math.floor((Math.random() > 0.5 ? 1 : -1) * Math.round(Math.random() * 25))
				arr[4 * mousePosToArr(x, y) + 2] =  Math.floor((Math.random() > 0.5 ? 1 : -1) * Math.round(Math.random() * 2))
			}
